...
PerceptronDemo/
  ...
  PerceptronDemoTests/             # Unit tests
    PerceptronDemoTests.swift      # Engine tests (pinned to the C# reference)
    PresetGeneratorTests.swift     # Trains + writes the bundled Presets/*.pcn files
...
- **Load Example ▸** — a submenu of bundled, pre-trained networks. Each item loads
  its `.pcn` directly (no picker).
...
- `PerceptronSnapshot` — `Codable` state: `gridSize`, `weights`, `bias`,
  `learningRate`, `switchStates`. `.fileExtension` is `"pcn"`; `jsonData()` emits
  pretty-printed, key-sorted JSON so regenerated presets diff cleanly.
- `Preset` — a `CaseIterable` enum cataloging the bundled examples. Each case
  maps to a `<rawValue>.pcn` in `Presets/`, plus a `menuTitle` and SF Symbol.
  `loadSnapshot(from:)` reads and decodes the bundled file.

**Presets are generated by a test, not hand-authored** (`PresetGeneratorTests.swift`):

- For each example it trains a fresh `PerceptronEngine` on a small,
  **linearly-separable** set of patterns (Top-vs-Bottom, Left-vs-Right, Diagonal,
  Density), **asserts** the trained network classifies every training sample
  correctly (a preset that can't be learned fails the build), and writes the
  result to `PerceptronDemo/Presets/<Name>.pcn` in the **source tree** (path
  derived from `#filePath`).
- Re-running `PresetGeneratorTests` regenerates the checked-in `.pcn` files from
  scratch — they are provably-trained fixtures. **The single-layer 1958
  perceptron only learns linearly-separable problems, so any new example must be
  linearly separable** (no XOR-style patterns) or the convergence assertion fails.
...
| Pre-trained examples | None | Bundled presets in gear-menu "Load Example ▸" |
