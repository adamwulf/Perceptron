...
final class PerceptronPanelViewController: UIViewController {
    ...
    private func makeGearMenu() -> UIMenu {
        ...
        return UIMenu(children: [save, load, about])
    }
    ...
    /// The full panel state written to / read from a `.pcn` file. Mirrors the
    /// desktop app's `PerceptronData`, plus the live switch pattern so a saved
    /// file restores exactly what was on screen.
    private struct PerceptronSnapshot: Codable {
        var gridSize: Int
        var weights: [Double]
        var bias: Double
    ...
    private static let fileExtension = "pcn"
    ...
    private func presentSave() {
        ...
        do {
            ...
            let filename = "perceptron_settings.\(Self.fileExtension)"
    ...
    private func presentLoad() {
        ...
        if let pcn = UTType(filenameExtension: Self.fileExtension) {
    ...
    private func loadSnapshot(from url: URL) {
        ...
        do {
            ...
            let snapshot = try JSONDecoder().decode(PerceptronSnapshot.self, from: data)
